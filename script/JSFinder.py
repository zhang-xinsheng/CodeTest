...
def parse_args():
    ...
    #parser.add_argument("-c", "--cookie", help="The cookie")
...
def Extract_html(URL,cookies=None):
	...
	head = {
		...
		#"Authorization:":"eyJ0eXBlIjoiSldUIiwiYWxnIjoiSFMyNTYifQ.eyJhdWQiOiJtdGctbW5nIiwic3ViIjoiMzEiLCJ0ZWFjaGVySWQiOiIzMSIsInVzZXJOYW1lIjoi5byg5biF5L6D77yI562J5L-d6K-E5Lyw5YWs5Y-477yJIiwiZXhwIjoxNTc4NTczNTM4fQ.hLJK0w2glYgtvynH88_VqHfIIjXfXFVujpAoNkb9pbM",
		"Cookie":cookies
	}
	...
	try:
		...
		raw = session.get(URL, headers=head, timeout=5, verify=False)

		#外网请求
		#raw = requests.get(URL, headers=header, timeout=3)
		
		charset = raw.apparent_encoding
		raw = raw.content.decode(charset, "ignore")
		#print(raw)
		return raw
	except:
		print('url请求出错！！！')
...
def find_by_url(url, cookies=None, js = False):
	if js == False:
		try:
			print("url:" + url)
		except:
		...
		#print('5')
		html_raw = Extract_html(url,cookies)
		if html_raw == None: 
		...
		for html_script in html_scripts:
			...
			if script_src == None:
				...
				script_array[purl] = Extract_html(purl,cookies)
		...
		return result
	return sorted(set(extract_URL(Extract_html(url,cookies)))) or None
...
print("[*]用法：在额外方框输入cookie字段（不需要Cookie头部）即可在后台界面收集敏感信息！否则只能在前台收集！")
def check(url,cookies=None):
	urls = find_by_url(url,cookies)
	giveresult(urls, url)
